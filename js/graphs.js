...
var GGraphs = (function(document) {
  ...
  GGraphs.prototype._drawLine = function(spline) {
    ...
    if (options.grid) {
      if (options.rotate) {
        headers.forEach(function(item) {
          rc = self._getTextSize(item);
          offsetBottom = Math.max(offsetBottom, rc.width);
          offsetRight = rc.height / 2;
        });
      } else {
        rc = self._getTextSize(headers[headers.length - 1]);
    ...
    let top = options.grid ? labelHeight / 2 : offsetRight,
      ...
      columnWidth = (this.width - offsetRight - x) / (headers.length - 1),
      panel = this.panel;
    if (options.grid) {
      ...
      headers.forEach(function(row) {
        ...
        if (options.rotate) {
          self.text(panel, x, bottom + top - pointerSize, row, self.fontColor, 'left', true);
        } else {
          self.text(panel, x, bottom + top + pointerSize, row, self.fontColor, 'center');
        }
        x += columnWidth;
      });
    }
  ...
  GGraphs.prototype._drawHChart = function() {
    ...
    if (options.grid) {
      headers.forEach(function(row) {
        self.line(panel, labelWidth + labeloffset, y, self.width - offsetRight, y, options.gridVColor, 1);
        self.text(panel, 0, y + middleRow, row, self.fontColor, 'left');
        y += cellHeight;
      });
      self.line(panel, labelWidth + labeloffset, y, self.width - offsetRight, y, options.gridVColor, 1);
      labels.forEach(function(row) {
  ...
  GGraphs.prototype._drawVChart = function() {
    ...
    if (options.grid) {
      ...
      headers.forEach(function(row) {
        self.line(panel, x, top, x, bottom, options.gridHColor, 1);
        if (options.rotate) {
          self.text(panel, x + midCell, bottom + top - pointerSize, row, self.fontColor, 'left', true);
        } else {
          self.text(panel, x + midCell, bottom + top + pointerSize, row, self.fontColor, 'center');
        }
        x += cellWidth;
      });
      self.line(panel, x, top, x, bottom, options.gridHColor, 1);
