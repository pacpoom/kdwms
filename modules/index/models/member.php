...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        ...
        //     }
        // }
        $select = array_merge($select, array('U.active', 'U.social', 'U2.role_name', 'U.status', 'U.create_date', 'U.lastvisited', 'U.visited', 'U.activatecode'));
        return static::createQuery()
            ->select($select)
            ->from('user U')
            ->join('role U2', 'LEFT',array('U.role_id', 'U2.id'))
            ->where($where);
    }
    ...
    public function action(Request $request)
    {
        ...
        if ($request->initSession() && $request->isReferer() && $login = Login::isAdmin()) {
            if (Login::notDemoMode($login)) {
                ...
                if ($action == 'addNew') {
                    // ลงทะเบียนสมาชิกใหม่
                    $ret['location'] = $request->getUri()->postBack('index.php', array('module' => 'register', 'id' => 0));
                } else {
                    if (preg_match_all('/,?([0-9]+),?/', $request->post('id')->filter('0-9,'), $match)) {
                        if ($action === 'delete') {
                            ...
                            $ret['location'] = $uri->createBackUri(array('module' => 'index-register', 'id' => 0));
