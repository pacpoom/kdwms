                ...
                try {
                    ...
                    if ($request->post('status')->toInt() == 0) {
                        ...
                        if ($request->post('location_code')->toString() == '') {
                            ...
                            $ret['location'] = $request->getUri()->postBack('index.php', array('module' => 'wms-shipcase',
                        ...
                        if ($location_code == '') {
                            ...
                            if (count($scan_qr) != 1) {
                                $ret['serial_number']='';
                                $ret['fault'] = Language::get('Please Scan Case ID !!');
                                $request->removeToken();
                            } else {
                                ...
                                if ($check_serial == false) {
                                    $ret['serial_number']='';
                                    $ret['fault'] = Language::get('Case ID Incorrect !!');
                                    $request->removeToken(); 
                                } else {
