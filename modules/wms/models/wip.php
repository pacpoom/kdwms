...
class model extends \Kotchasan\Model{
    ...
    public static function toDataTable($params){
        ...
        ->select('T1.id','T2.material_number','T2.material_name_en','T2.material_name_thai','T1.quantity','T5.unit','T2.unit_price','0 Amount','T3.currency')
        ->from('inventory_wip T1')
        ->join('material T2','LEFT',array('T1.material_id','T2.id'))
        ->join('unit T5','LEFT',array('T2.unit','T5.id'))
        ->join('currency T3','LEFT',array('T3.id','T2.currency'))
        ->where(array('T1.quantity','!=',0))
        ->order('id');
    }
