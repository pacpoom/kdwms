...
class model extends \Kotchasan\Model{
    ...
    public static function saledetail($params){
        ...
        if (!empty($params['so'])) {
            $where[] = array('sale_order',$params['so']);
        } 

        if (!empty($params['customer'])) {
            $where[] = array('customer_code',$params['customer']);
        }
    ...
    public static function get_detail($params){
        ...
        return static::createQuery()
        ->select('T1.id','T1.sale_order','T1.material_number','T3.serial_number','T3.actual_quantity'
        ,'T4.location_code','T2.serial_number pick','T1.ship_date','T1.pallet_no','T1.truck_id')
