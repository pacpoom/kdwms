...
class Model extends \Kotchasan\Model{
    ...
    public static function getContainer($raw){
        return static::createQuery()
        ->select('T1.id','T1.container','T1.temp_material','T2.inbound_date','T2.serial_number','T2.actual_quantity','T4.location_code')
        ...
        ->where(array(array('T1.box_id',$raw)))
        ->order('T2.inbound_date');
    ...
    public static function get_Data(){
        ...
        return static::createQuery()
        ->select('T1.id','T1.serial_number')
        ->from('inventory_stock T1')
        ->where(array(array('T1.actual_quantity','!=',0)))
        ->order('T1.serial_number')
        ->groupBy('T1.id','T1.serial_number')
        ->execute();
    }
    ...
    public static function sumstock($id){
        return static::createQuery()
        ->select('T1.id',sql::SUM('T2.actual_quantity','qty'))
        ->from('packing_list T1')
        ->join('inventory_stock T2','LEFT',array('T1.id','T2.reference'))
        ->where(array(array('T1.box_id',$id),array('T2.actual_quantity','!=',0)))
        ->groupBy('T1.id')
        ->execute();
    }
            ...
            if (Login::notDemoMode($login)) {
                try {
                    ...
                    if ($request->post('raw')->toString() == '') {
                        ...
                        $scan_qr = explode("_",$request->post('raw')->topic());
                        ...
                        $total_sum = \wms\querybox\Model::sumstock($request->post('raw')->toString());

                        $ret['location'] = $request->getUri()->postBack('index.php', array('module' => 'wms-querybox', 
                        'total_box' => 1,
                        'total_qty' => 1,
                        'raw' => $raw,
