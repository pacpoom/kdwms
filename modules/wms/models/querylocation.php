...
class Model extends \Kotchasan\Model{
    ...
    public static function getContainer($raw){
        ...
        ->where(array(array('T4.location_code',$raw),array('T1.actual_quantity','!=',0)))
    ...
    public static function get_Data(){
        ...
        return static::createQuery()
        ->select('T1.location_id','T2.location_code')
        ->from('inventory_stock T1')
        ->join('location T2','LEFT',array('T1.location_id','T2.id'))
        ->where(array(array('T1.actual_quantity','!=',0)))
        ->order('T2.location_code')
        ->groupBy('T1.location_id','T2.location_code')
        ->execute();
    }
        ...
        if ($request->initSession() && $request->isReferer() && $login = Login::isMember()) {
            if (Login::notDemoMode($login)) {
                try {
                    ...
                    if ($request->post('raw')->toString() == 0) {
                        $ret['raw']='';
                        $ret['ret_raw'] = 'Please fill in';
                    } else {

                        $total_sum = \wms\querylocation\Model::sumstock($request->post('raw')->toString());

                        $ret['location'] = $request->getUri()->postBack('index.php', array('module' => 'wms-querylocation', 
