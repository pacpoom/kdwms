...
class Controller extends \Gcms\Controller{
    ...
    public function render(request $request){
        ...
        $container_no = \wms\caset\Model::getContainerID($container);
        
        if ($container_no == false) {
            $section->appendChild(\wms\caset\View::create()->render($total_box,$total_receive,0,$status,$location_code));
            $section->appendChild(\wms\caset\View::create()->show_data($request,NULL,NULL));
            

        } else {
            $section->appendChild(\wms\caset\View::create()->render($total_box,$total_receive,$container_no[0]->container,$status,$location_code));
            $section->appendChild(\wms\caset\View::create()->show_data($request,$container_no[0]->container,$location_code));
