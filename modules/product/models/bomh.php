...
class model extends \Kotchasan\Model{
    ...
    public static function toDataTable($params){
        ...
        ->select('T1.model_no','T2.material_number','T2.material_name_en','T2.material_type','T4.unit')
        ->from('bom T1')
        ->join('material T2','LEFT',array('T1.model_no','T2.id'))
        ->join('unit T4','LEFT',array('T2.unit','T4.id'))
        ->groupBy('T1.model_no','T2.material_number','T2.material_name_en','T2.material_type','T4.unit')
                ...
                if ($action ==='add'){
                    ...
                    if (preg_match_all('/,?([0-9]+),?/', $request->post('id')->filter('0-9,'), $match)) {
                        ...
                        if ($action === 'delete') {
                            ...
                            $index = $request->post('id')->toString();
    
                            //var_dump($index);

                            $ret['location'] = $request->getUri()->postBack('index.php', array('module' => 'product-bom', 
                            'model' => $index)); 
