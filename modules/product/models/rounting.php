...
class model extends \Kotchasan\Model{
    ...
    public static function toDataTable(){
        ...
        ->select('T1.id','T1.routing_id','T2.material_number','T2.material_name_en','T2.material_type','T1.min','T1.max','T4.unit')
        ->from('routing T1')
        ...
        if ($request->initSession() && $request->isReferer() &&  $login = Login::isMember()) {
            if (Login::notDemoMode($login) && Login::checkPermission($login, 'can_manage_inventory')) {
                ...
                if ($action ==='add'){
                    ...
                    $index = \product\rounting\Model::get($request->post('id')->toInt());

                    $ret['modal'] = Language::trans(\product\rountings\View::create()->render($request,0,$index));
                    
                } elseif ($action == 'statusd') {
                    $index = \product\rounting\Model::get($request->post('id')->toInt());
                    $ret['modal'] = Language::trans(\product\rountings\View::create()->render($request,1,$index));
                } else{
